...
/* ------------ PRINT HELPERS ------------ */
async function fetchInvoiceFull(id) { const { data } = await api.get(`/api/invoices/purchases/${id}`); return data; }
...
<header>${company.logo ? `<img src="${company.logo}" alt="Logo"/>` : ""}<div class="brand"><div class="name">${company.name || ""}</div><div class="muted">Purchase Invoice</div></div></header>
<h1>Invoice #${inv.invoiceNo || ""}</h1>
<div class="muted">${new Date(inv.date || inv.createdAt || Date.now()).toLocaleString()}</div>
<div style="margin-top:8px"><div><strong>Supplier:</strong> ${inv.supplier?.name || ""}</div>
<div class="muted">${inv.supplier?.phone || ""} ${inv.supplier?.address ? " | " + inv.supplier.address : ""}</div></div>
...
export default function PurchaseSummary() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [sup, cats] = await Promise.all([api.get("/api/suppliers"), api.get("/api/categories")]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mapped = list.map((d) => {
        ...
        return {
          id: d._id, invoiceNo: d.invoiceNo, date: d.date || d.createdAt, party: d.supplier?.name || "",
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = "purchase-summary.csv"; a.click(); URL.revokeObjectURL(url);
  };

  const clearFilters = () => { setSupplierId(""); setFrom(""); setTo(""); setSearch(""); setCategoryName(""); setPage(1); fetchData(); };
              ...
              {current.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="px-3 py-2 text-right">
                    <Button variant="ghost" onClick={async () => { try { const inv = await fetchInvoiceFull(r.id); printInvoice(inv); } catch { alert("Failed to load invoice for printing."); } }} title="Print invoice">Print</Button>
                  </td>
                </tr>
              ))}
