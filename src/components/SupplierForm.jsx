...
export default function SupplierForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr(e?.response?.data?.message || "Failed to load supplier");
      }
    })();
  }, [id, isEdit]);
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded-2xl shadow border">
      <h1 className="text-2xl font-bold mb-4">{isEdit ? "Edit Supplier" : "Add Supplier"}</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button variant="soft" onClick={() => navigate("/suppliers")}>Cancel</Button>
        </div>
      </form>
    </div>
  );
