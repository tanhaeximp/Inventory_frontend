...
async function fetchInvoiceFull(id) {
  const { data } = await api.get(`/api/invoices/sales/${id}`);
  return data;
}
...
<header>
  ...
  <div class="brand"><div class="name">${company.name || ""}</div><div class="muted">Sales Invoice</div></div>
</header>
...
<div style="margin-top:8px">
  ...
  <div class="muted">${inv.customer?.phone || ""} ${inv.customer?.address ? " | " + inv.customer.address : ""}</div>
</div>
...
export default function SalesSummary() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [cus, cats] = await Promise.all([
  ...
  // pagination
  const totalPages = Math.max(1, Math.ceil(baseFiltered.length / 15));
  const current = baseFiltered.slice((page - 1) * 15, page * 15);
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
      ...
      <Card className="p-4 mb-4">
        {/* Mobile: 1 col, md: 2 cols, lg: 12 cols to avoid overlap */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
          ...
          <div className="lg:col-span-4 min-w-0">
            ...
            <div className="flex gap-2 items-stretch min-w-0">
              ...
              />
              <Button variant="soft" onClick={clearFilters} className="whitespace-nowrap">
                Clear
              </Button>
            </div>
          </div>
      ...
      <Card className="overflow-hidden">
        ...
        {/* Pagination */}
        {!loading && baseFiltered.length > 15 && (
          <div className="flex items-center justify-between p-3 border-t">
            <div className="text-sm text-gray-600">
              Showing{" "}
              <span className="font-medium">
                {(page - 1) * 15 + 1}–{Math.min(page * 15, baseFiltered.length)}
              </span>{" "}
