// src/pages/PurchaseInvoicePage.jsx
...
<title>${t("purchaseInvoice")} ${invoice.invoiceNo || ""}</title>
<style>
...
export default function PurchaseInvoicePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [sup, prod, cats] = await Promise.all([
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setErr(e?.response?.data?.message || "Failed to create purchase invoice");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl text-white p-6 shadow">
          <div className="flex flex-col md:flex-row md:items-end gap-4">
            <div className="flex-1">
              <div className="text-sm/5 text-white/80">{t("purchaseInvoice") || "Purchases Invoice"}</div>
              <h1 className="text-3xl font-bold tracking-tight">
                {supplierId ? ` ${suppliers.find((s) => s.value === supplierId)?.label || ""}` : ""}
              </h1>
            </div>
          </div>
        </div>
