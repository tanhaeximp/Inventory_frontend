...
export default function Suppliers() {
  ...
  const onAdd = () => navigate("/suppliers/new");
  const onEdit = (s) => navigate(`/suppliers/${s._id}`);
  const onDelete = async (s) => {
    if (!window.confirm(`${t("delete") || "Delete"} "${s.name}"?`)) return;
    try { await api.delete(`/api/suppliers/${s._id}`); fetchSuppliers(); }
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl text-white p-6 shadow">
          <div className="flex flex-col md:flex-row md:items-end gap-4">
            <div className="flex-1">
              <div className="text-sm/5 text-white/80">{t("suppliers") || "Suppliers"}</div>
              <h1 className="text-3xl font-bold tracking-tight">{t("suppliersList") || "Suppliers List"}</h1>
            </div>
            <div className="flex gap-2">
              <Button variant="soft" onClick={fetchSuppliers} disabled={loading}>
        ...
        <ResponsiveTableCards
          ...
          renderRow={(s, idx) => (
            <tr key={s._id || idx}>
              ...
              <td className="px-3 py-2 text-right">{Number(s.balance || 0).toFixed(2)}</td>
              <td className="px-3 py-2">
                <div className="flex justify-end gap-2">
                  <Button variant="soft" onClick={() => onEdit(s)}>{t("edit") || "Edit"}</Button>
                  <Button variant="danger" onClick={() => onDelete(s)}>{t("delete") || "Delete"}</Button>
                </div>
              </td>
            </tr>
          )}
          renderCard={(s, idx) => (
            <div key={s._id || idx} className="rounded-xl border p-3 bg-white space-y-1">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <div className="font-semibold tabular-nums">{Number(s.balance || 0).toFixed(2)}</div>
              </div>
